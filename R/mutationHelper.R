...
# Each element is composed by a mutation df and a vector with all patient barcodes for that tumor type
                            ...
                            , block=NULL)
{
    ...
    if(tumor_type[1]=="all_tumors") {
        ...
        if (length(chosenTumors)==0){
          stop("Error: could not find tumor_type nor cancer_study_id in .getMutations()")
        }
    }
    out_double <- lapply(chosenTumors , function(i)
        {
            ...
            # in 99% of the cases mutations are present but in case, just return NULL
            if(!any(sel)){
            ...
            if(any(sel)) {
                if(is.null(block)){
                    message(paste("getting mutations from this cancer study:" , i ))
                } else {
                    message(paste("getting mutations from this cancer study:" , i , paste0("(" , block , ")")))
                }
                ...
                error <- tryCatch(
                    muts <- cgdsr::getMutationData( mycgds 
                        ...
                        , genes=myGenes)
                    , error=function(e) {
                        message(paste("Impossible to retrive mutations from" , i , "study"))
                        }
                    )
                ...
                # message(paste("No mutations data from" , i , "study"))
                muts <- NULL
                patients <- NULL
            }
...
.subsetMutations <- function(panel , muts_full , rs_df)
{
    ...
    if(any(panel$exact_alteration=="amino_acid_position")){
        ...
        if(any(!is.na(mut_data1$case_id))){
            ...
            mut_data1 <- mut_data1[!is.na(mut_data1$amino_position) , , drop=FALSE]
            mut_data1 <- mut_data1[mut_data1$amino_position>=mut_data1$start & mut_data1$amino_position<=mut_data1$end , , drop=FALSE] %>% unique
        } else {
            mut_data1 <- emptyDF
        }        
        # invisible(lapply(c("start" , "end") , function(x) mut_data1[ , x] <<- NULL))
    ...
    if(any(panel$exact_alteration=="amino_acid_variant")){
        ...
        mut_data2 <- merge(muts_full , panel_mut_2 , all.y=TRUE , by="gene_symbol")
        if(any(!is.na(mut_data2$case_id))){
            mut_data2 <- mut_data2[ !is.na(mut_data2$case_id) , ]
            mut_data2 <- mut_data2[mut_data2$amino_acid_change==mut_data2$mutation_specification , , drop=FALSE] %>% unique
        } else {
            mut_data2 <- emptyDF
        }
        # invisible(lapply("mutation_specification" , function(x) mut_data2[ , x] <<- NULL))
    ...
    if(any(panel$exact_alteration=="mutation_type")){
        ...
        mut_data3 <- merge(muts_full , panel_mut_3 , all.y=TRUE , by="gene_symbol")
        if(any(!is.na(mut_data3$case_id))){
            mut_data3 <- mut_data3[ !is.na(mut_data3$case_id) , ]
            mut_data3 <- mut_data3[(mut_data3$mutation_type %in% kmiss_type & mut_data3$mutation_specification=="missense") |
                        (mut_data3$mutation_type %in% ktrunc_type & mut_data3$mutation_specification=="truncating"), , drop=FALSE] %>%
                        unique
        } else {
            mut_data3 <- emptyDF    
        }
        mut_data3 <- mut_data3[ , colnames(emptyDF)]
        # invisible(lapply("mutation_specification" , function(x) mut_data3[ , x] <<- NULL))
    } else {
        mut_data3 <- emptyDF
    }

    if(any(panel$exact_alteration=="genomic_position" | panel$exact_alteration=="dbSNP_rs")){
        panel_mut_4 <- panel[ panel$exact_alteration %in% c("genomic_position","dbSNP_rs")
                            , c("drug", "gene_symbol" , "mutation_specification" ,"group")
                            ]
        if(!is.null(rs_df)){
            panel_mut_4$mutation_specification <- .mapvalues(panel_mut_4$mutation_specification 
                                                    , from=rs_df$rs
        ...
        if(any(!is.na(mut_data4$case_id))){
            ...
            mut_data4 <- mut_data4[ mut_data4$genomic_position_num>=mut_data4$start & 
                                mut_data4$genomic_position_num<=mut_data4$end , ] %>%
                                 unique
            # invisible(lapply(c("start" , "end" , "genomic_position_num") , function(x) mut_data4[ , x] <<- NULL))
        } else {
            mut_data4 <- emptyDF
        }
    ...
    if(any(panel$exact_alteration=="genomic_variant")){
        ...
        mut_data5 <- merge(muts_full , panel_mut_5 , all.y=TRUE , by="gene_symbol")
        if(any(!is.na(mut_data5$case_id))){
            mut_data5 <- mut_data5[!is.na(mut_data5$case_id) , ]
            mut_data5 <- mut_data5[ mut_data5$genomic_variant==mut_data5$mutation_specification, ] %>% unique
        } else {
            mut_data5 <- emptyDF
        }
        mut_data5 <- mut_data5[ , colnames(emptyDF)]
        # invisible(lapply("mutation_specification" , function(x) mut_data5[ , x] <<- NULL))
    } else {
        mut_data5 <- emptyDF
    }
    ...
    if(any(panel$exact_alteration=="")){
        ...
        mut_data6 <- merge(muts_full , panel_mut_6 , all.y=TRUE , by="gene_symbol")
        ...
        # invisible(lapply("mutation_specification" , function(x) mut_data6[ , x] <<- NULL))
    } else {
        mut_data6 <- emptyDF
    }
