  ...
  for(i in names(kColnamesIndataFull)) {
    ...
    if(is.null(repos[[i]]$Samples) & is.null(repos[[i]]$data)) {
      ...
      if(any(repos[[i]]$data$case_id %notin% unlist(repos[[i]]$Samples))) {
        stop(paste( "In" , i , ", some case_id are in the data but not in the samples"))
      }
      if(any(repos[[i]]$data$tumor_type %notin% names(repos[[i]]$Samples))) {
        stop(paste( "In" , i , ", some tumor_type are in the data but are not listed in the samples"))
      }
    }
 }
}
...
setGeneric('getAlterations', function(object, tumor_type=NULL
						, repos=NULL
						, mutation_type=c("all_nonsynonymous" , "all_mutations" , "missense" , "truncating")
						, expr_z_score=2
...
setMethod('getAlterations', 'CancerPanel', function(object, tumor_type=NULL
						, repos=NULL
						, mutation_type=c("all_nonsynonymous" , "all_mutations" , "missense" , "truncating")
						, expr_z_score=2
						, BPPARAM=bpparam("SerialParam")
						...
						)
{
  ...
  # Freq is not calculated
	if(!is.null(repos)) {
		...
		object@dataFull$copynumber <- list(data=repos$copynumber$data)
		object@dataFull$expression <- list(data=repos$expression$data)
		tumor_type <- c()
		for(i in c("mutations" , "copynumber" , "expression" , "fusions")){
			...
			if(is.null(repos[[i]]$Samples)){
				if(!is.null(repos[[i]])) {
					...
					tumor_type <- unique(c(tumor_type , unique(repos[[i]]$tumor_type)))
  ...
  if(is.null(tumor_type) || tumor_type == ""){
	...
	if(!is.numeric(expr_z_score)){
		stop("Expression z score should be a numeric value (good values can be 2 or 3)")
	}
  ...
  if(!is.null(tumor_type) && tumor_type[1]!="all_tumors"){
	  ...
	  #get list of cancer studies available
		...
		if( any( tumor_type %notin% availtumtype ) & any( tumor_type %notin% availcancerstudy )){
  ...
  #-----------------------------------------------------------------------------
	if(tumor_type[1]=="all_tumors"){
	  # derived_tumor_type <- showTumorType() %>% unname
	  # object@arguments$tumor_type <- showTumorType() %>% unname
  ...
  #duplicate panel
	...
	if(any(object@arguments$panel$alteration=="fusion")) {
		...
		if(nrow(fus)==0){
			...
			# samples_fus <- split(fus$case_id , fus$tumor_type) %>% lapply(. , unique)
			samples_fus <- fusSamples[names(fusSamples) %in% derived_tumor_type]
			if(any(derived_tumor_type %notin% names(fusSamples))){
				missingFusSamples <- derived_tumor_type[derived_tumor_type %notin% names(fusSamples)]
				samples_fus_miss <- lapply(missingFusSamples , function(x) NULL) %>% setNames(missingFusSamples)
				samples_fus <- c(samples_fus , samples_fus_miss)
			}
			fus <- fus[ fus$tier %in% c("tier1" , "tier2") , ]
			fus <- fus[ fus$frame!="Out-of-frame" , ]
			fusgenes <- panel[ panel$alteration=="fusion" , "gene_symbol"] %>% unique
			gtOut <- fus[ fus$Gene_A %in% fusgenes | fus$Gene_B %in% fusgenes | fus$FusionPair %in% fusgenes , ]
			# if(nrow(gtOut)!=0){
			# 	gtGenesTab <- table(gtOut$FusionPair , factor(gtOut$tumor_type , levels=derived_tumor_type))
			# } else {
			# 	gtGenesTab <- NULL
			# }
	...
	if(any(object@arguments$panel$alteration=="SNV")) {
	  ...
	  availability <- .checkDataAvailability(tumor_type=tumor_type , genProfile="mutations$") %>% 
		...
		if(all(availability==0)){
			...
			mutgenes <- panel[ panel$alteration=="SNV" , "gene_symbol"] %>% unique
			# There is a bug in cgdsr: when genes like C10orf12 are queried, they are put in upper case
			# The solution is to revert them to their original case
			mutgenes_translator <- data.frame(original=mutgenes,upper=toupper(mutgenes),stringsAsFactors = FALSE)
			...
			if(is.null(gmOut2)){
				...
				if(mutation_type=="missense")
					gmOut2 <- gmOut2[ gmOut2$mutation_type %in% kmiss_type , ]
				if(mutation_type=="truncating")
					gmOut2 <- gmOut2[ gmOut2$mutation_type %in% ktrunc_type , ]
				if(mutation_type=="all_mutations")
					gmOut2 <- gmOut2[ gmOut2$mutation_type %in% ktcga_types , ]
				# Add new useful columns
        # EXTRACT ALSO POSITION FORM SPLICE SITES
				gmOut2$amino_position <- ifelse( gmOut2$mutation_type %notin% knotTransc 
				                                 , as.numeric(as.character(str_extract(string=gmOut2$amino_acid_change,pattern="\\d+")))
				                                 , NaN)
				# gmOut2$amino_position <- ifelse( gmOut2$mutation_type %notin% c("Splice_Site" , knotTransc) , 
				# 									as.numeric(as.character(str_extract(string=gmOut2$amino_acid_change,pattern="\\d+"))) , 
				# 									NaN)
				#-----------------------------
				...
				if(any(is.na(gmOut2$chr))){
					...
					ensembl <- biomaRt::useMart(host="feb2014.archive.ensembl.org"
												,biomart="ENSEMBL_MART_ENSEMBL"
												,dataset="hsapiens_gene_ensembl")
					Chr_df <- biomaRt::getBM(mart=ensembl
											,values=nochr
											,filters="hgnc_symbol"
				...
				#
				gmOut2$genomic_position <- paste(gmOut2$chr
												, gmOut2$start_position 
												,paste(gmOut2$reference_allele , gmOut2$variant_allele , sep=",")
												,sep=":")
				invisible(lapply(c("chr" , "start_position" , "reference_allele" , "variant_allele") , function(x)
									gmOut2[ , x] <<- NULL))
				...
				if(any(mutgenes_translator$original!=mutgenes_translator$upper)){
				  ...
				  gmOut2$gene_symbol <- .mapvalues(gmOut2$gene_symbol 
				...
				# gmGenes <- factor(gmOut2$gene_symbol , levels=mutgenes)
				# gmTumor_type <- factor(gmOut2$tumor_type , levels=unique(gmOut2$tumor_type) )
				# gmGenesTab <- data.frame(gmGenes=gmGenes , case_id=gmOut2$case_id , tumor_type=gmTumor_type) %>%
				# 				unique
				# gmGenesTab <- table(gmGenesTab$gmGenes , gmGenesTab$tumor_type)
				# gmGenesTab <- sapply(colnames(gmGenesTab) , function(x) gmGenesTab[ , x]/length(samples_mut[[x]]) )
	...
	if(any(object@arguments$panel$alteration=="CNA")) {
		...
		if(all(availability==0)){
			...
			cnagenes <- panel[ panel$alteration=="CNA" , "gene_symbol"] %>% unique
			# There is a bug in cgdsr: when genes like C10orf12 are queried, they are put in upper case
			# The solution is to revert them to their original case
			cnagenes_translator <- data.frame(original=cnagenes,upper=toupper(cnagenes),stringsAsFactors = FALSE)
			# cgdsr performs poorly on large queries
			# If the number of requested genes is too large we subset the SQL query in chunks
			if(length(cnagenes)>100) {
				...
				gcOut_melt <- lapply(gcOut , function(tobereturned) {
								for(x in names(tobereturned)) {
									if(!is.null(tobereturned[[x]]$out)) {
										tobereturned[[x]]$out$case_id <- gsub("\\." , "-" , rownames(tobereturned[[x]]$out))
										tobereturned[[x]]$out$genetic_profile_id <- x
										tum_type <- strsplit(x , "_")[[1]][1]
										tobereturned[[x]]$out$tumor_type <- tum_type
									}
								}
								tobereturned <- as.data.frame(rbindlist(lapply(tobereturned , '[[' , 1)))
								tobereturned_melt <- reshape2::melt(tobereturned , id.vars=c("case_id" , "genetic_profile_id" , "tumor_type")
										,value.name="CNA" , variable.name="gene_symbol"
				...
				for(x in names(gcOut)) {
					if(!is.null(gcOut[[x]]$out)) {
						gcOut[[x]]$out$case_id <- gsub("\\." , "-" , rownames(gcOut[[x]]$out))
				...
				gcOut_melt <- reshape2::melt(gcOut2 , id.vars=c("case_id" , "genetic_profile_id" , "tumor_type")
									,value.name="CNA" , variable.name="gene_symbol"
			...
			if(any(cnagenes_translator$original!=cnagenes_translator$upper)){
			  cnagenes_translator <- cnagenes_translator[ cnagenes_translator$original!=cnagenes_translator$upper , , drop=FALSE]
			  gcOut_melt$gene_symbol <- .mapvalues(gcOut_melt$gene_symbol 
			                                            , from=cnagenes_translator$upper 
			                                            , to=cnagenes_translator$original
			                                            , warn_missing = FALSE)
			}
			# All this mess is because gistic applies different scores at different datasets
			# This create confusion, because if the same patient belongs to different study
			# Its gistic copynumber can be different
			gcOut_melt2 <- data.frame(tumor_type=factor(gcOut_melt$tumor_type , levels=derived_tumor_type)
			...
			gcOut_melt2 <- aggregate(CNA~tumor_type+gene_symbol+case_id , gcOut_melt2 , mean)
			gcOut_melt2$CNA <- factor(as.character(round(gcOut_melt2$CNA)) , levels=c("2" , "1" , "0" , "-1" , "-2"))
			# NOTE: the format of Freq is different from mutations. It is a list (not a matrix)
			# It contains an element for each tumor type
			# Each tumor type is a dataframe with amplification, normal and deletion freqeuncy
			# gcGenesTab <- table(gcOut_melt2$gene_symbol , gcOut_melt2$CNA , gcOut_melt2$tumor_type)
			# amp_table <- lapply(dimnames(gcGenesTab)$tumor_type , function(i) {
			# 					tab <- gcGenesTab[ , , i]
			# 					if(length(cnagenes)>1)
			# 						df <- data.frame(amplification=tab[ , "2"]/length(samples_cna[[i]])
			# 									,normal=rowSums(tab[ , c("1" , "0" , "-1")])/length(samples_cna[[i]])
			# 									,deletion=tab[ , "-2"]/length(samples_cna[[i]])
			# 									,row.names=rownames(gcGenesTab[ , , i]))
			# 					else
			# 						df <- data.frame(amplification=tab[ "2"]/length(samples_cna[[i]])
			# 									,normal=sum(tab[ c("1" , "0" , "-1")])/length(samples_cna[[i]])
			# 									,deletion=tab[ "-2"]/length(samples_cna[[i]])
			# 									,row.names=cnagenes) 
			# 					return(df)
			# 				})
			# names(amp_table) <- dimnames(gcGenesTab)$tumor_type
			gcOut_melt3 <- data.frame(gene_symbol=as.character(gcOut_melt2$gene_symbol)
									, CNA=ifelse(as.character(gcOut_melt2$CNA) %in% c("1" , "0" , "-1") , "normal" 
	...
	if(any(object@arguments$panel$alteration=="expression")) {
		...
		if(all(availability==0)){
			...
			exprgenes <- panel[ panel$alteration=="expression" , "gene_symbol"] %>% unique
			# There is a bug in cgdsr: when genes like C10orf12 are queried, they are put in upper case
			# The solution is to revert them to their original case
			exprgenes_translator <- data.frame(original=exprgenes,upper=toupper(exprgenes),stringsAsFactors = FALSE)
			# cgdsr performs poorly on large queries
			# If the number of requested genes is too large we subset the SQL query in chunks
			if(length(exprgenes)>100) {
				...
				geOut_melt <- lapply(geOut , function(tobereturned) {
							for(x in names(tobereturned)) {
								if(!is.null(tobereturned[[x]]$out)) {
									tobereturned[[x]]$out$case_id <- gsub("\\." , "-" , rownames(tobereturned[[x]]$out))
									tobereturned[[x]]$out$genetic_profile_id <- x
							...
							tobereturned <- as.data.frame(rbindlist(lapply(tobereturned , '[[' , 1)))
							tobereturned_melt <- reshape2::melt(tobereturned , id.vars=c("case_id" , "genetic_profile_id" , "tumor_type")
									,value.name="expression" , variable.name="gene_symbol"
									,factorsAsStrings=FALSE)
							return(tobereturned_melt)
							})
				...
				for(x in names(geOut)) {
					if(!is.null(geOut[[x]]$out)) {
						geOut[[x]]$out$case_id <- gsub("\\." , "-" , rownames(geOut[[x]]$out))
						geOut[[x]]$out$genetic_profile_id <- x
						tum_type <- strsplit(x , "_")[[1]][1]
						geOut[[x]]$out$tumor_type <- tum_type
					}
				}
				geOut2 <- as.data.frame(rbindlist(lapply(geOut , '[[' , 1)))
				geOut_melt <- reshape2::melt(geOut2 , id.vars=c("case_id" , "genetic_profile_id" , "tumor_type")
									,value.name="expression" , variable.name="gene_symbol"
									,factorsAsStrings=FALSE) %>% .changeFactor(.)
			...
			if(any(exprgenes_translator$original!=exprgenes_translator$upper)){
			  exprgenes_translator <- exprgenes_translator[ exprgenes_translator$original!=exprgenes_translator$upper , , drop=FALSE]
			  geOut_melt$gene_symbol <- .mapvalues(geOut_melt$gene_symbol 
			                                            , from=exprgenes_translator$upper 
			                                            , to=exprgenes_translator$original
			                                            , warn_missing = FALSE)
			}
			geOut_melt2 <- unique(geOut_melt[ , c("tumor_type" , "case_id" , "gene_symbol" , "expression")])
			geOut_melt2$expression <- .noNA(geOut_melt2$expression , subs=0)
			geOut_melt2 <- aggregate(expression~tumor_type+gene_symbol+case_id , geOut_melt2 , mean)
			geOut_melt2$expressionValue <- geOut_melt2$expression
			expression_dichotomize <- ifelse(geOut_melt2$expression >= expr_z_score , "up" 
										, ifelse(geOut_melt2$expression <= (-expr_z_score) , "down" 
											, "normal")
										)
			geOut_melt2$expression <- factor(expression_dichotomize , levels=c("up" , "normal" , "down"))
			# NOTE: the format of Freq is different from mutations. It is a list (not a matrix)
			# It contains a element for each tumor type
			# Each tumor type is a dataframe with amplification, normal and deletion frequency
			# geGenesTab <- table(geOut_melt2$gene_symbol , geOut_melt2$expression , geOut_melt2$tumor_type)
			# expr_table <- lapply(dimnames(geGenesTab)$tumor_type , function(i) {
			# 					tab <- geGenesTab[ , , i]
			# 					if(length(exprgenes)>1)
			# 						df <- data.frame(up=tab[ , "up"]/length(samples_expr[[i]])
			# 									,normal=tab[ , "normal"]/length(samples_expr[[i]])
			# 									,down=tab[ , "down"]/length(samples_expr[[i]])
			# 									,row.names=rownames(geGenesTab[ , , i]))
			# 					else
			# 						df <- data.frame(up=tab[ "up"]/length(samples_expr[[i]])
			# 									,normal=tab[ "normal"]/length(samples_expr[[i]])
			# 									,down=tab[ "down"]/length(samples_expr[[i]])
			...
			# 					return(df)
			# 				})
			# names(expr_table) <- dimnames(geGenesTab)$tumor_type
			geOut_melt3 <- data.frame(gene_symbol=as.character(geOut_melt2$gene_symbol)
