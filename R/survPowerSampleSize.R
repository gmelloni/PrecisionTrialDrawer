...
.surv2Samps <- function(alpha = 0.05 , beta = NULL , sample.size = NULL 
    ...
    if(mode == "sample.size"){
        events <- ((qnorm(1-alpha/side)+qnorm(1-beta))/(log(hr)-log(hr0)))^2/
          (case.fraction*(1-case.fraction))
        return(events/p.event)
    } else if(mode == "power"){
        z <- (log(hr)-log(hr0))*sqrt(sample.size*case.fraction*
                                       (1-case.fraction)*p.event)
        return( pnorm(z - qnorm(1-alpha/side)) + 
                  pnorm( -z - qnorm(1-alpha/side)) )
    } else {
...
.calPowerSamp <- function(finalTabFrac , sample.size , HR , HR0 , power 
                          , alpha , case.fraction , round.result 
                          , ber , fu , acc = NULL , side){
    # print(finalTabFrac)
        ...
        for(i in seq_len(length(HR))){
            ...
            sample.size <- lapply(beta , function(x) {
                    .surv2Samps(alpha = alpha , beta = x , hr = hr , hr0 = hr0 
                        , case.fraction = case.fraction , mode = "sample.size" 
                        , ber = ber , fu = fu , acc = acc , side=side)
                    }) %>% unlist
    ...
    if(is.null(power)){
        ...
        for(i in seq_len(length(HR))){
            ...
            power <- lapply(as.vector(sample.size) , function(x) {
                .surv2Samps(alpha = alpha , sample.size = x 
                    , hr = hr , hr0 = hr0 
                    , case.fraction = case.fraction , mode = "power" 
                    , ber=ber , fu=fu , acc=acc , side=side)
            }) %>% unlist
            # print(power)
            beta <- 1 - power
...
, noPlot=FALSE)
{
    ...
    side <- side[1]
    requiredParam <- list(HR=HR , HR0=HR0 , case.fraction=case.fraction 
                          , alpha=alpha , ber=ber , fu=fu , side=side)
    ...
    if(("alteration_id" %in% var) & collapseByGene){
        warning("If 'alteration_id' is var, you cannot collapse by gene.")
        collapseByGene <- FALSE
    }
    ...
    if(!is.null(priority.trial)){
        ...
        if(length(priority.trial)<2){
            stop(paste("priority.trial must be a character vector"
                       ,"containing at least 2 elements"))
        }
        if(var %notin% c("drug" , "group")){
            stop(paste("priority.trial parameter can be set"
                       ,"only if var is drug or group"))
        }
        if(var=="drug"){
      ...
      if(any(duplicated(priority.trial))){
        ...
        if(is.null(power)){
            stop(paste("priority.trial can be used only to"
                       ,"calculate sample size, power cannot be NULL"))
        }
    }
    #----------------------------
    ...
    # If tumor.freqs is set, we basically run this 
    # method in recursive mode for every tumor type
    ...
    if(!is.null(tumor.freqs)){
        if(!is.na(var) && var=="tumor_type"){
            ...
            survRecurse <- lapply(names(tumor.freqs) , function(tum){
                ...
                finalTabFracTum <- aggregate(as.formula( paste(Rate , "~ Var") )
                                             , out , FUN=mean)
            ...
            toBePlot <- .calPowerSamp(
              finalTabFrac=survRecurseWeighted_vec , sample.size 
              , HR , HR0 , power , alpha , case.fraction 
              , round.result=round.result , ber=ber , fu=fu 
              , acc=acc , side=side)
        }
    } else {
    ...
    if(!is.null(priority.trial)) {
        powerHRMat <- data.frame(HazardRatio = rep(HR , length(power))
                ...
                , Power = lapply( power , function(x) rep(x , length(HR))) %>% 
                  unlist
                , stringsAsFactors = FALSE)
        priorTrial <- lapply(seq_len(nrow(powerHRMat)) , function(row) {
            ...
            # This matrix represents the order 
            # of j to follow after the recruitment
            matForOrder <- diag(1 , nrow=k , ncol=k)
            for(i in seq_len(nrow(matForOrder))){
                matForOrder[ i , which(matForOrder[i,]!=1)] <- 2:k
            }
            # i start a new run of Screening, j take care of the 
            # discarded, to check if they can fit into another drug
                    ...
                    if(i == j){
                        ...
                        if(i == 1){
                            ...
                            if(sum(matSampsAssigned[ , j])>=TSS){
                                ...
                                # Here, we deal with new screenings. A new 
                                # screening X is calculated 
                                # as X = (TSS - already assigned)/p
                        ...
                        if(matSamps[i , j_old]>=0){
                            ...
                            sampToRemove <- myDataForTotal[ 
                              myDataForTotal[ , var] %in% drugsDone 
                              , "case_id"] %>% unique
                            ...
                            if(which(vec==j) == k){
                                ...
                                if( all( colSums(matSampsAssigned[  
                                  ,  setdiff(names(matSampsAssigned), drugsDone)
                                  , drop=FALSE]) >= TSS )){
                                    matSampsAssigned[ i , j ] <- sampObtainable
                                } else {
                                    if(sum(matSampsAssigned[ , j])>=TSS){
                                        ...
                                        # If samples obtainable are 
                                        # not sufficient to reach TSS
                                        # , assign 'em all
                                        # If they are sufficient, assigned just 
                                        # what is enough to reach TSS
                            ...
                            # Remaining samples are the ones from 
                            # the previous cycle - what was assigned
            ...
            matSampsAssignedWithTotal <- rbind( matSampsAssigned 
                                                , colSums(matSampsAssigned))
            ...
            discard <- matSampsComplete[ 
              , ncol(matSampsComplete)] - 
        ...
        names(priorTrial) <- apply( powerHRMat , 1 , function(x) {
          paste( paste(c("HR" , "HR0" , "Power") , x , sep=":") 
                 , collapse=" | ")
          })
        if(noPlot){
    ...
    if(noPlot){
        ...
        plotTitle <- "Screening sample size by power stratified by HR levels"
        ...
        p <- ggplot(toBePlot , aes(y=Power, x=ScreeningSampleSize
                                   , colour=HazardRatio , group=HazardRatio)) +
